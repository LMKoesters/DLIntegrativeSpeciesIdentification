...
# with minor adjustments for compatibility, a custom classifier for SequentialEmbeddingBarcodeJPGBar
# and an additional layer transforming fractional encoded barcodes to ordinal encoding
...
# barcode model
class SequentialEmbeddingBarcodeJPGBar(nn.Module):
    def __init__(self, num_classes, classifier_mode, init_weights=True, w_classifier=True):
        super(SequentialEmbeddingBarcodeJPGBar, self).__init__()
        self.block = SequentialBottleneck
        self.inplanes = 64
        self._norm_layer = nn.BatchNorm2d
        self.dilation = 1
        self.groups = 1
        self.base_width = 64
        # embedding layer to get from one-hot encoded to sequential
    ...
    def make_embedding(self):
        embedding_layers = [nn.Conv2d(4, 1, kernel_size=1, stride=1, padding=0, bias=False)]
        embedding_layers += [nn.ReLU()]
    ...
    def make_resnet(self, block):
        ...
        model_layers = [nn.Conv2d(1, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)]
        ...
        model_layers += [self._make_layer(block, 128, layers[1], stride=1)]
        model_layers += [self._make_layer(block, 256, layers[2], stride=1)]
        model_layers += [self._make_layer(block, 512, layers[3], stride=1)]

        return nn.Sequential(*model_layers)

    def _make_layer(
            self,
            block: Type[Union[SequentialBottleneck]],
            planes: int,
            blocks: int,
            stride: int = 1,
            dilate: bool = False,
