...
#' @param model_method Specify the model family used for the glm
#' @param model_summaries Whether to return summaries of the simplified models
...
simplify_model <- function(glm_model, aic_bic=FALSE, model_method='binomial',
  ...
  if (p_values_model[1, 1] < 0.1) {
    ...
    for (i in 2:length(p_values_model$coefficients)) {
      ...
      no_more_simplification <- anova_res$`Pr(>Chi)`[2] < .05
      ...
      if (no_more_simplification & simplification_history[[i-1]]$p_values[1, 2] < 0.1) {
        final_model <- simplification_history[[i-1]]
        break
      }
    }

    if (simplification_history[[length(simplification_history)]]$p_values[1, 2] < 0.1) {
      final_model <- simplification_history[[length(simplification_history)]]
    }
  }

  if (!is.null(final_model)) {
    p_values_model <- simplification_history[[length(simplification_history)]]$p_values
    marginal_sign_vars <- p_values_model[(p_values_model$p_values < 0.1) & (p_values_model$p_values >= 0.05),]
    sign_vars <- p_values_model[p_values_model$p_values < 0.05,]

    final_model$significant_variables <- sign_vars$coefficients
    final_model$marginally_significant_variables <- marginal_sign_vars$coefficients
  }
